...
日英両方を提供するドキュメントでは、このことをタイトル直後に宣言してから内容に移る。日本語版だけを持つ内部文書には宣言を要求しない。
...
セクションヘッダの言語や日英ファイルの同時更新など、構文・差分から判定できる項目は機械的に検証してよい。日英内容の意味的一致は AI または人間がレビューし、必須 CI にはしない。
