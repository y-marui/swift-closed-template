...
### 2.4 Relationship to dev-charter Dev-Env Topics

対応する `*_DEV_ENV.md`（`PYTHON_DEV_ENV.md`・`ALFRED_DEV_ENV.md` 等、[CHARTER_INDEX.md](../CHARTER_INDEX.md) 参照）が存在する技術スタックの場合、複数プロジェクトに共通する一般方針（バージョン管理・Lint/Format・アーキテクチャ方針等）はそのファイルを正本とし、テンプレート側に重複して記載しない。テンプレートの `AI_CONTEXT.md`・`README-jp.md` からは該当する dev-env トピックを参照するだけにとどめる。

テンプレート固有の技術リファレンス（実際のエクスポート・実機検証からしか再生成できないもの。例: Alfred の `info.plist` オブジェクトスキーマ）は、対応する dev-env トピック側には書かず、テンプレート自身の docs を正本とする。dev-charter 側が一般原則として記載してよいのは「テンプレート側の docs を参照する」という分離方針までであり、具体的なリポジトリ名・パスへの固定参照は憲章に含めない（憲章に追加できるのは複数プロジェクトに適用可能な原則までであり、特定リポジトリ名は 1 プロジェクト固有の情報にあたる）。どのリポジトリが対応テンプレートかという対応表は、各プロジェクトの `AI_CONTEXT.md` または利用者個人の恒久ドキュメント側で管理する。
