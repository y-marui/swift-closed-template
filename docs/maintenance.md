...
```
このプロジェクトの docs/architecture.md の更新案を作ってください。
...
4. 以下のフォーマットで更新案を、根拠となるファイルパスとともに出力する：
...
- 更新案には根拠となるファイルパス・該当箇所・不確実な点を含める
```
...
```
このプロジェクトの docs/file-map.md の更新案を作ってください。

手順：
1. 最近変更または参照したファイルを特定する
2. 既存の docs/file-map.md を読む（存在する場合）
3. 以下のフォーマットで更新案を、根拠となるファイルパスとともに出力する（未探索ファイルは追加しない）：
